...
//inorder in sorted order traversal;
void inorder(Node* root,vector<int>& ans){ //only root node.
    if(root==NULL) return;  //base case
    inorder(root->left,ans); //recursion of left part of tree 01
    ans.push_back(root->val);   //work
    inorder(root->right,ans); //recursion of right part of tree 02
}
...
int main(){
    ...
    display(a);
    vector<int>ans;
    inorder(a,ans); //sorted inc seq;
    levelOrderQueue(a,ans); //min heap;
    display(a);
}
